...
public static class MineTests
{
    public static void Run(Action<bool,string> check)
    {
        ...
        try
        {
            ...
            foreach(var id in Mine.Ids)g.Rooms!.Rooms.Remove(id);
            foreach(var l in RoomLinks.All.Skip(15))g.Rooms!.Doors.Remove(l.Id);
            g.Rooms!.LayoutVersion=6;g.Health=73;Save();
            check(g.Rooms!.LayoutVersion==8&&g.Rooms.Rooms.Count==19&&g.Health==73&&g.RegimentState.FarledReached,"Published regiment saves gain unexplored mine without resetting story or health");
