...
public static class MineTests
{
    public static void Run(Action<bool,string> check)
    {
        ...
        try
        {
            ...
            foreach(var fresh in Observatory.Ids.Concat(Gamla.Ids))g.Rooms!.Rooms.Remove(fresh);foreach(var fresh in RoomLinks.All.Where(l=>l.Id.StartsWith("observatory-")||(l.Id.StartsWith("gamla-")||l.Id.StartsWith("salt-"))))g.Rooms!.Doors.Remove(fresh.Id);g.Rooms!.LayoutVersion=6;g.Health=73;Save();
            check(g.Rooms!.LayoutVersion==15&&g.Rooms.Rooms.Count==37&&g.Health==73&&g.RegimentState.FarledReached,"Published regiment saves gain unexplored mine without resetting story or health");
