...
public static class CabinTests
{
    public static void Run(Action<bool,string> check)
    {
        ...
        try
        {
            g.Rooms!.Rooms.Remove(Cabin.Room);foreach(var fresh in Observatory.Ids)g.Rooms.Rooms.Remove(fresh);foreach(var fresh in RoomLinks.All.Where(l=>l.Id.StartsWith("observatory-")))g.Rooms.Doors.Remove(fresh.Id);g.Rooms.LayoutVersion=10;Save();check(g.Rooms!.Rooms.Count==27&&g.Health==21&&g.MeridianState.OrderTaken,"Published Meridian endpoint migrates without resetting wounds or story");
