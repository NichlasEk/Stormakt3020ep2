...
public static class UppsalaTests
{
    public static void Run(Action<bool,string> check)
    {
        foreach(var weapon in new[]{Weapon.Saber,Weapon.Hammer})foreach(var order in new[]{Order.Artillery,Order.Medicine})
        {
            ...
            try
            {
                foreach(var id in Uppsala.Ids)g.Rooms!.Rooms.Remove(id);g.Rooms!.Doors.Remove("uppsala-clock");g.Rooms.Doors.Remove("meridian-hall");foreach(var fresh in Observatory.Ids.Concat(Gamla.Ids))g.Rooms.Rooms.Remove(fresh);foreach(var fresh in RoomLinks.All.Where(l=>l.Id.StartsWith("observatory-")||(l.Id.StartsWith("gamla-")||(l.Id.StartsWith("salt-")||l.Id.StartsWith("rescue-")))))g.Rooms.Doors.Remove(fresh.Id);g.Rooms.LayoutVersion=8;g.Health=73;Save();check(g.Rooms!.LayoutVersion==16&&g.Rooms.Rooms.Count==41&&g.Health==73,"Published foundry save gains Uppsala without healing");
