...
public static class ObservatoryTests
{
    public static void Run(Action<bool,string> check)
    {
        foreach(var weapon in new[]{Weapon.Saber,Weapon.Hammer})foreach(var order in new[]{Order.Artillery,Order.Medicine})
        {
            ...
            try
            {
                g.Rooms!.LayoutVersion=11;foreach(var id in Observatory.Ids)g.Rooms.Rooms.Remove(id);foreach(var l in RoomLinks.All.Where(l=>l.Id.StartsWith("observatory-")))g.Rooms.Doors.Remove(l.Id);Save();check(g.Rooms!.LayoutVersion==12&&g.Rooms.Rooms.Count==27&&g.CabinState.Briefed,"Old cabin save migrates without resetting story");
