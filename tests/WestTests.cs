...
public static class WestTests
{
    public static void Run(Action<bool,string> check)
    {
        foreach(var order in new[]{Order.Artillery,Order.Medicine})foreach(var weapon in new[]{Weapon.Saber,Weapon.Hammer})
        {
            ...
            try
            {
                foreach(var id in West.Ids)g.Rooms!.Rooms.Remove(id);foreach(var l in RoomLinks.All.Where(l=>l.Id.StartsWith("gamla-west-")))g.Rooms!.Doors.Remove(l.Id);g.Rooms!.LayoutVersion=13;Save();check(g.Rooms.LayoutVersion==14&&g.Rooms.Rooms.Count==33&&g.GamlaState.Debriefed,"Published Gamla save migrated");
