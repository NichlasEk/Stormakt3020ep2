...
public static class RegimentTests
{
    public static void Run(Action<bool,string> check)
    {
        ...
        foreach(var fresh in Observatory.Ids.Concat(Gamla.Ids))legacy.Rooms!.Rooms.Remove(fresh);foreach(var fresh in RoomLinks.All.Where(l=>l.Id.StartsWith("observatory-")||l.Id.StartsWith("gamla-")))legacy.Rooms!.Doors.Remove(fresh.Id);legacy.Rooms!.LayoutVersion=5;legacy.Rooms.ConnectionRevision=3;legacy.Health=61;
        ...
        try
        {
            ...
            check(restored.Rooms!.LayoutVersion==14&&restored.Rooms.Rooms.Count==33,"Published nine-room saves gain six unvisited rooms");
