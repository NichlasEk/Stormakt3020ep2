...
static class WaterTests
{
    public static void Run(Action<bool,string> check)
    {
        ...
        try
        {
            ...
            node["Rooms"]!.AsObject().Remove("LayoutVersion");var dict=node["Rooms"]!["Rooms"]!.AsObject();dict.Remove(PortRooms.Pump);dict.Remove(PortRooms.Cistern);dict.Remove(PortRooms.Gallery);dict.Remove(PortRooms.Chamber);dict.Remove(PortRooms.Archive);dict.Remove(PortRooms.Roots);dict.Remove(PortRooms.Grove);
            var migrated=Save(node.Deserialize<Combat>(options)!,"legacy");
            check(migrated.Rooms!.LayoutVersion==5&&migrated.Rooms.Rooms.Count==9&&migrated.Enemies[0].Health==42&&!migrated.Rooms.Rooms[PortRooms.Pump].Visited,"Old two-room save gains unopened rooms without resetting progress");
