...
public static class GamlaTests
{
    public static void Run(Action<bool,string> check)
    {
        foreach(var order in new[]{Order.Artillery,Order.Medicine})
        {
            ...
            try
            {
                foreach(var id in Gamla.Ids)g.Rooms!.Rooms.Remove(id);foreach(var l in RoomLinks.All.Where(l=>l.Id.StartsWith("gamla-")))g.Rooms!.Doors.Remove(l.Id);g.Rooms!.LayoutVersion=12;Save();
                check(g.Rooms!.LayoutVersion==14&&g.Rooms.Rooms.Count==33&&g.ObservatoryState.Debriefed,"Observatory saves gain Gamla Uppsala");
