...
public static class FoundryTests
{
    public static void Run(Action<bool,string> check)
    {
        ...
        try
        {
            foreach(var id in Uppsala.Ids)run.Rooms!.Rooms.Remove(id);run.Rooms!.Doors.Remove("uppsala-clock");run.Rooms.Doors.Remove("meridian-hall");run.Rooms.Rooms.Remove(Foundry.Room);run.Rooms.Doors.Remove("foundry");foreach(var fresh in Observatory.Ids)run.Rooms.Rooms.Remove(fresh);foreach(var fresh in RoomLinks.All.Where(l=>l.Id.StartsWith("observatory-")))run.Rooms.Doors.Remove(fresh.Id);run.Rooms.LayoutVersion=7;run.Health=68;Save();
            check(run.Rooms!.LayoutVersion==12&&run.Rooms.Rooms.Count==27&&!run.Rooms.Rooms[Foundry.Room].Visited&&run.Health==68,"Published mine save gains unopened foundry");
