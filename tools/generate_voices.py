...
if '--mine-only' in sys.argv: lines=json.loads((ROOT/'assets/story/mine-radio.json').read_text())
...
for role,v in roles.items():
 ...
 for name,(r,line) in lines.items():
  ...
  if '--mine-only' in sys.argv or ('--retake-regiment' in sys.argv and name in {'regiment-captain','regiment-freed','regiment-marshal'}):
   parts=[]
