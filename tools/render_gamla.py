"""Render the Gamla Uppsala arrival shot through the verified local LTX graph and GPU broker."""
...
shots={'arrival':'A solemn eight second arrival shot at the ancient Swedish royal burial mounds. A huge dark blue and gold royal spacecraft already hovering at the left settles very slightly, engine lights dim gently, a breath of dust moves under its hull and low morning mist passes across the empty muster yard. Keep the stone entrance and grassy mounds perfectly rigid. The camera gently moves toward the dark entrance under the mound, no cuts. Preserve the old matte oil painted texture and exact layout. No people, no text, no sails, no new objects, no morphing. Audio: quiet fading engine rumble, low northern wind and one distant raven, no music and no speech.'}
for index,(name,prompt) in enumerate(shots.items()):
    dest=SRC/f'gamla-{name}-ltx-v1.mp4'
    if dest.exists():print('Already rendered',name,flush=True);continue
    input_name=f'ep2-gamla-{name}-v1.png';shutil.copyfile(ROOT/'assets/art/room-gamla-landing-v1.png',PROFILE/'input'/input_name)
    ...
    (SRC/f'gamla-{name}-workflow-v1.json').write_text(json.dumps(g,indent=2))
    lease=api(broker,'/v1/gpu/jobs',{'owner':'Stormakt3020ep2','owner_id':'gamla-'+name,'label':'Gamla Uppsala '+name,'kind':'video','ttl_seconds':3600})
    try:
        for _ in range(120):
        ...
        job=api(base,'/prompt',{'prompt':g,'client_id':'stormakt-ep2-gamla-rest'})
        (SRC/f'gamla-{name}-job-v1.json').write_text(json.dumps(job,indent=2));print(name,job,flush=True)
        for i in range(360):
            h=api(base,'/history/'+job['prompt_id'])
            if h:
                (SRC/f'gamla-{name}-history-v1.json').write_text(json.dumps(h,indent=2))
