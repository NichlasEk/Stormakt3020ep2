...
journey='--journey' in sys.argv or rooms or archive or roots or intro
journey_ids={'voice-'+key for key in json.loads((root/('assets/story/intro-radio.json' if intro else 'assets/story/rootway-radio.json' if roots else 'assets/story/archive-radio.json' if archive else 'assets/story/rooms-radio.json' if rooms else 'assets/story/journey-radio.json')).read_text())}
report=root/('artifacts/intro-voice-transcripts.json' if intro else 'artifacts/rootway-voice-transcripts.json' if roots else 'artifacts/archive-voice-transcripts.json' if archive else 'artifacts/rooms-voice-transcripts.json' if rooms else 'artifacts/journey-voice-transcripts.json' if journey else 'artifacts/voice-transcripts.json')
