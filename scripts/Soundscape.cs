...
public partial class Soundscape : Node
{
    ...
    public override void _Ready()
    {
        ...
        foreach(var id in JourneyDialogue.Rooms().Keys.Concat(JourneyDialogue.Archive().Keys).Concat(JourneyDialogue.Roots().Keys).Concat(JourneyDialogue.Regiment().Keys).Concat(JourneyDialogue.Mine().Keys).Concat(JourneyDialogue.Foundry().Keys).Concat(JourneyDialogue.Uppsala().Keys).Concat(JourneyDialogue.Continuity().Keys).Concat(JourneyDialogue.Observatory().Keys).Concat(JourneyDialogue.Cabin().Keys).Concat(JourneyDialogue.Meridian().Keys))
            if(ResourceLoader.Exists($"res://assets/audio/voice-{id}.ogg"))_clips["voice-"+id]=GD.Load<AudioStream>($"res://assets/audio/voice-{id}.ogg");
