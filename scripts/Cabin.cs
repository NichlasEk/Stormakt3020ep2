...
public sealed partial class Combat
{
    ...
    private bool StepCabin(Func<Vector2,bool> near)
    {
        ...
        if(near(Cabin.Talk))
        {
            ...
            else if(WestState.ElinMet&&!WestState.Debriefed){WestState.Debriefed=true;Emit("radio",Player,"west-debrief");Emit("radio",Player,"west-aboard");Emit("radio",Player,"west-next");Emit("campaign",Player,"ELIN ÄR OMBORD: Hon får vila medan expeditionen ordnar återföreningen med Märta. Saltkällan står i handlingen som nästa destination för de andra. Vem som beställde namnbytena vet vi ännu inte.");}
            ...
            else Emit("radio",Player,WestState.Debriefed?"west-next":GamlaState.Debriefed?"gamla-debrief":ObservatoryState.Debriefed?"observatory-next":"cabin-repeat");
            Emit("room-sound",Player,"paper");Emit("checkpoint",Player);return true;
        }
        if(WestState.ElinMet&&near(West.Aboard)){Emit("radio",Player,"west-aboard");return true;}
        if(near(Cabin.Rest))
        {
