...
{
    private enum Screen { Title, Briefing, Game, Pause, Settings, Death, Ending, Testimony, Journal, Inventory, Archive, Cinematic, Videos, Music }
    ...
    private readonly MusicDirector _musicDirector=new();
    private string SavePath=>ProjectSettings.GlobalizePath(_rescueSlot?"user://rescue-preview-save.json":_saltSlot?"user://salt-preview-save.json":_westSlot?"user://west-preview-save.json":_gamlaSlot?"user://gamla-preview-save.json":_observatorySlot?"user://observatory-preview-save.json":_meridianSlot?"user://meridian-preview-save.json":_uppsalaSlot?"user://uppsala-preview-save.json":_foundrySlot?"user://foundry-preview-save.json":_mineSlot?"user://mine-preview-save.json":_regimentSlot?"user://regiment-preview-save.json":_doorSlot?"user://door-trial-save.json":_roomsSlot?"user://rooms-save.json":_portSlot?"user://port-save.json":_atlandSlot?"user://atland-save.json":"user://quay-save.json");
    private string ManualPath=>ProjectSettings.GlobalizePath(_rescueSlot?"user://rescue-preview-manual-save.json":_saltSlot?"user://salt-preview-manual-save.json":_westSlot?"user://west-preview-manual-save.json":_gamlaSlot?"user://gamla-preview-manual-save.json":_observatorySlot?"user://observatory-preview-manual-save.json":_meridianSlot?"user://meridian-preview-manual-save.json":_uppsalaSlot?"user://uppsala-preview-manual-save.json":_foundrySlot?"user://foundry-preview-manual-save.json":_mineSlot?"user://mine-preview-manual-save.json":_regimentSlot?"user://regiment-preview-manual-save.json":_doorSlot?"user://door-trial-manual-save.json":_roomsSlot?"user://rooms-manual-save.json":_portSlot?"user://port-manual-save.json":_atlandSlot?"user://atland-manual-save.json":"user://manual-save.json");
    ...
    public override void _Ready()
    {
        ...
        if(args.Contains("--music-check")){RunMusicChecks();return;}
        if(args.Contains("--rescue-check")){RunRescueChecks();return;}
        if(args.Contains("--rescue")||args.Contains("--rescue-new")){StartRescue(args.Contains("--rescue-new"));return;}
        if(args.Contains("--salt-check")){RunSaltChecks();return;}
        if(args.Contains("--salt")||args.Contains("--salt-new")){StartSalt(args.Contains("--salt-new"));return;}
        ...
        if(args.Contains("--observatory")||args.Contains("--observatory-new")){StartObservatory(args.Contains("--observatory-new"));return;}
        ...
        if(args.Contains("--mine")||args.Contains("--mine-new")){StartMine(args.Contains("--mine-new"));return;}
        if(args.Contains("--regiment-check")){RunRegimentChecks();return;}
        if(args.Contains("--ports-check")){RunRegimentChecks(true);return;}
        if(args.Contains("--regiment")||args.Contains("--regiment-new")){StartRegiment(args.Contains("--regiment-new"));return;}
        if(args.Contains("--root-arch-check")){RunConnectedChecks(false,true);return;}
        if(args.Contains("--arch-check")){RunConnectedChecks(true);return;}
    ...
    private void Back()
    {
        ...
        else if(_screen==Screen.Game)ChangeScreen(Screen.Pause);
        ...
        else if(_screen is Screen.Videos or Screen.Music)ChangeScreen(Screen.Settings);
    ...
    {
        if(id.StartsWith("music:",StringComparison.Ordinal)){_previewScore=id[6..];return;}
        ...
        switch(id)
        {
            ...
            case "meridian-preview":StartMeridian();break;
            ...
            case "continue":_rescueSlot=false;_saltSlot=false;_westSlot=false;_gamlaSlot=false;_observatorySlot=false;_meridianSlot=false;_uppsalaSlot=false;_shipTime=0;_pendingStoryFilm="";_radioBreath=0;_foundrySlot=false;_mineSlot=false;_regimentSlot=false;_doorSlot=false;_roomsSlot=false;_portSlot=false;_atlandSlot=false;ResumeSave();break;
    ...
    private void ResumeSave(bool manual=false)
    {
        try
        {
            _game=SaveStore.Read(manual?ManualPath:SavePath);_pendingStoryFilm="";_radioBreath=0;_radioTime=0;if(_game.InDoorTrial)LoadDoorArt();if(_game.InRooms){LoadWaterArt();if(!_testMode)_game.EnableConnectedWorld();}if(!_game.InCampaign)_game.PreferRoomRoute=true;ApplyDeveloperSettings();_camera=G(_game.Player)+new Vector2(85,-80);_particles.Clear();_floating.Clear();_radioQueue.Clear();_radio="";_sound.StopVoice();
            if(_game.InCampaign){_campaignText=_game.InRooms?_game.RoomGoal:_game.Stage.Intro;_campaignTextTime=10;_revealTime=0;}
            ChangeScreen(_game.Dead?Screen.Death:_game.Phase==Phase.Complete?Screen.Ending:_game.Phase==Phase.Testimony?Screen.Testimony:Screen.Game);Notice("Fältdagboken återupptagen");QueueFarewell();
        }
    ...
    public override void _PhysicsProcess(double delta)
    {
        ...
        if(_boatTime>0||_shipTime>0||_pendingStoryFilm!=""){ClearPresses();return;}
    ...
    {
        if(_game.InRooms&&!_game.CanSeeRoomPoint(cue.Position)&&cue.Kind is "loot" or "warning" or "shot" or "hit" or "scrape" or "swing" or "death" or "block")return;
        var p=G(cue.Position);
        switch(cue.Kind)
        {
            case "hero-change":_selectedItem=-1;_inventoryPage=0;_camera=G(_game.Player)+new Vector2(0,-60);_particles.Clear();_floating.Clear();RememberRenderPositions();if(_game.IsEbba){_radioQueue.Clear();_radio="";_radioTime=0;_sound.StopVoice();}break;
            case "rescue-reunion":_pendingStoryFilm="rescue-reunion";break;
            ...
            case "room-sound":
                if(NVec.Distance(_game.Player,cue.Position)<500)_sound.Play(cue.Text);
                if(cue.Text=="oath-impact"&&_game.CanSeeRoomPoint(cue.Position)){Burst(p,Gold,20,80);_shake=3;}
                if(cue.Text=="mine-steam"&&_game.CanSeeRoomPoint(cue.Position))
                    for(int i=0;i<22;i++){float life=.5f+i%5*.09f;_particles.Add(new(){P=p+new Vector2((i%7-3)*6,-8),V=new Vector2((i%5-2)*12,-90-i%4*13),Life=life,Max=life,C=new Color(.68f,.7f,.66f),Size=5+i%4});}
                break;
            ...
            case "gamla-travel":StartGamlaTravel(cue.Text);break;
            case "painted-arrival":_camera=G(_game.Player)+new Vector2(0,-60);RememberRenderPositions();_particles.Clear();_floating.Clear();break;
    ...
    private void QueueRadio(string id)
    {
        ...
        if(id=="meridian-fallen")
        {
            bool CombatLine(string key)=>key is "meridian-fight" or "meridian-tactic" or "meridian-rage";
            var keep=_radioQueue.Where(key=>!CombatLine(key)).ToArray();_radioQueue.Clear();foreach(var key in keep)_radioQueue.Enqueue(key);
            if(CombatLine(_radio)){_sound.StopVoice();_radio="";_radioTime=0;}
        }
        if(id=="meridian-repeat"||!_radioQueue.Contains(id))_radioQueue.Enqueue(id);
    }
    ...
    public override void _Process(double delta)
    {
        ...
        _sound.Boss=(_game.Phase==Phase.Collector||(_game.Phase==Phase.Extraction&&_game.Enemies.Any(e=>!e.Dead))||(_game.InCampaign&&_game.Enemies.Any(e=>!e.Dead&&e.Kind is (EnemyKind.Collector or EnemyKind.OathGuardian or EnemyKind.RootMarshal or EnemyKind.CrownBailiff or EnemyKind.MeridianWarden or EnemyKind.ZenithGuardian or EnemyKind.MusterOfficer or EnemyKind.SaltWarden or EnemyKind.Censor)&&_game.CanSeeRoomPoint(e.Position)))) && _screen is Screen.Game or Screen.Pause;
        _sound.MusicPreview=_screen==Screen.Music;
        if(_screen==Screen.Music)_sound.SelectScore(_previewScore);else _sound.SelectScore(_musicDirector.Select(_game,_sound.Boss,_boatTime>0,_shipTime>0,_screen is Screen.Title or Screen.Briefing));
        ...
        if(_screen is Screen.Game or Screen.Ending or Screen.Testimony or Screen.Archive)
        {
            ...
            if(_radioTime<=0 && !_sound.Speaking && _radioQueue.Count>0&&_radioBreath<=0){_radio=_radioQueue.Dequeue();_radioTime=Radio.TryGetValue(_radio,out var line)?Math.Max(8,line.Text.Length/15f):8;_sound.Speak(_radio);_radioBreath=_radio=="continuity-dead"?3:0;}
    ...
    public override void _Draw()
    {
        ...
        else if(_screen==Screen.Game){DrawHud();if(_game.PassageCurtain>0)DrawRect(new Rect2(0,0,1280,720),new Color(0,0,0,_game.PassageCurtain));if(_game.InDoorTrial)DrawDoorHelp();}
        ...
        else if(_screen==Screen.Music)DrawMusicMenu();
        else if(_screen==Screen.Videos)DrawVideoMenu();
    ...
    private void DrawWorld()
    {
        ...
        foreach(var h in _game.Hazards.Where(h=>_game.CanSeeRoomPoint(h.Position)||(_game.InRooms&&NVec.Distance(h.Position,_game.Player)<h.Radius+60&&_game.ClearPath(_game.Player,h.Position))))
        {var c=h.Steam?new Color("c4bba2"):h.Friendly?Gold:Red;DrawCircle(G(h.Position),h.Radius,new Color(c,.10f));DrawArc(G(h.Position),h.Radius,0,Mathf.Tau,70,c,2,true);DrawArc(G(h.Position),h.Radius*Math.Clamp(1-h.Timer/1.5f,0,1),0,Mathf.Tau,60,new Color(c,.5f),2,true);}
        foreach(var e in _game.Enemies.Where(e=>!e.Dead && e.State==1&&_game.CanSeeRoomPoint(e.Position)))DrawTelegraph(e);
        DrawDepthSortedActors();
        DrawDroppedItems();
        foreach(var shot in _game.Shots.Where(s=>_game.CanSeeRoomPoint(s.Position))){var p=G(shot.Position);var v=G(shot.Velocity).Normalized();DrawLine(p-v*17,p,shot.Reflected?Teal:Gold,3,true);DrawCircle(p,3,Pale);}
    ...
    {
        if(e.Kind is EnemyKind.MusterOfficer or EnemyKind.SaltWarden or EnemyKind.Censor)return;
        ...
        if(e.Kind==EnemyKind.MeridianWarden){foreach(var strikePoint in _game.MeridianStrikePoints(e)){DrawArc(G(strikePoint),52,0,Mathf.Tau,40,Gold,2,true);DrawLine(G(e.Position),G(strikePoint),new Color(Gold,.5f),1.5f,true);}return;}
        if(e.Kind==EnemyKind.RootMarshal){DrawArc(G(e.LockedAim),65,0,Mathf.Tau,40,Gold,2,true);DrawLine(G(e.Position),G(e.LockedAim),new Color(Red,.6f),2,true);return;}
        if(e.Kind==EnemyKind.OathGuardian){DrawOathTelegraph(e);return;}
    ...
    {
        if(e.Kind==EnemyKind.Censor){DrawCensor(e);return;}
        if(e.Kind==EnemyKind.SaltWarden){DrawSaltWarden(e);return;}
        ...
        if(e.Kind is EnemyKind.RootMarshal or EnemyKind.RootSoldier){DrawRegimentActor(e);return;}
        if(e.Kind==EnemyKind.OathGuardian){DrawOathActor(e);return;}
    ...
    {
        if(_game.IsEbba){DrawPlayableEbba();return;}
    ...
    {
        float actorScale=player?_game.PassageScale:1;var actorOffset=Offset+p*Zoom*(1-actorScale);
        var shadow=new Vector2[24];for(int i=0;i<24;i++)shadow[i]=p+new Vector2(Mathf.Cos(i*Mathf.Tau/24)*25,Mathf.Sin(i*Mathf.Tau/24)*10)*actorScale;
        DrawColoredPolygon(shadow,new Color(.01f,.02f,.025f,dead?.2f:.42f));
        if(player&&_game.Passage==null)DrawArc(p,23,0,Mathf.Tau,40,new Color(Teal,.5f),1.5f,true);
        if(kind is "karl-saber" or "guard")
        {
            ...
            _animated.Draw(this,player?"karl":"guard",p,facing,action,frame,hurt,dead,actorOffset,Zoom*actorScale);
        }
        else if(kind is "karl-hammer" or "collector" or "pikeman" or "gunner" && moving&&pose<3&&hurt<=0&&!dead)
            _animated.Draw(this,kind,p,player?PlayerWalkFacing():facing,"walk",Gait.Frame(walk),0,false,actorOffset,Zoom*actorScale);
        else _cast.Draw(this,kind,p,facing,pose,hurt,dead,actorOffset,Zoom*actorScale);
        DrawSetTransform(Offset,0,Vector2.One*Zoom);
    }
    private void DrawHud()
    {
        ...
        if(_game.DeveloperSurvival){Panel(new Rect2(20,132,240,29),.9f);Text("DEV · Överlever på 1 liv",new Vector2(30,152),12,Gold);}
        ...
        int foes=_game.Enemies.Count(e=>!e.Dead&&_game.CanSeeRoomPoint(e.Position));
        ...
        Panel(new Rect2(417,623,470,77),.96f);Text(_game.WeaponName.ToUpperInvariant()+(_game.Weapon==Weapon.Pistol&&_game.RescueState.PistolReload>0?$" · {_game.RescueState.PistolReload:0.0}s":""),new Vector2(435,647),16,Gold);
        ...
        else
        {
            ...
            else if(!_game.InDoorTrial&&_game.Elapsed<35&&(!_game.InCampaign||_campaignTextTime<=0)){Panel(new Rect2(282,552,716,43),.85f);Centered(_game.InCabin?(_controller?"Vänster spak Gå · B Tala / undersök":"WASD Gå · E Tala / undersök"):_game.Weapon==Weapon.Pistol?(_controller?"X Skjut · Y Riktat skott · A Undan · LB Parad":"WASD Gå · Mus Sikta · Vänster Skjut · Höger Riktat · Space Undan"):_controller?"X Hugg · Y Tungt · A Undanmanöver · LB Parad":"WASD Gå · Mus Sikta · Vänster Hugg · Höger Tungt · Space Undan · Shift Parad",640,578,14,Muted);}
        }
    ...
    {
        if(_game.InDoorTrial||_game.InConnectedWorld)return;
    ...
    private void DrawRadio()
    {
        ...
        else if(speaker=="SIGILLMÄSTAREN")DrawTextureRectRegion(_censorArt!,portraitRect,new Rect2(320,20,145,145));
        else if(speaker=="KARL CCLV")DrawTextureRectRegion(_karlRescue!,portraitRect,new Rect2(210,20,115,115));
        else if(speaker=="SALTVÄKTAREN")DrawTextureRectRegion(_saltArt!,portraitRect,new Rect2(320,20,150,150));
        ...
        else if(speaker.StartsWith("KAPTEN")){var size=_captainArt!.GetSize();DrawTextureRectRegion(_captainArt,portraitRect,new Rect2(size.X*.41f,size.Y*.02f,size.X*.22f,size.Y*.22f));}
        else if(portrait==0)DrawTextureRect(_ebbaPortrait,portraitRect,false);
    ...
    private void DrawSettings()
    {
        ...
        Button(new Rect2(880,329,320,49),"Utvecklare · Musikbibliotek","music-library");
        ...
        Centered("Spelaren tar skada men överlever på minst 1 liv.",640,534,15,Muted);
        Button(new Rect2(205,550,420,43),"Utvecklarverktyg · Videobibliotek","videos");
        ...
        Button(new Rect2(845,652,240,34),"Prova Meridiansalen","meridian-preview");
    ...
    {
        Centered("Havet väntar",640,235,46,Pale,true);Centered((_game.IsEbba?"Ebba":"Karl")+" föll. Fältdagboken finns kvar.",640,279,18,Muted);
    ...
    {
        _grovePassageArt?.Dispose();_courtOpenArt?.Dispose();_lodgePassageArt?.Dispose();_cisternOpenArt?.Dispose();_cabinArt?.Dispose();_ebbaCabin?.Dispose();_uppsalaArt?.Dispose();_quayFrigate?.Dispose();_flightArt?.Dispose();_bailiffArt?.Dispose();_bailiffWalk?.Dispose();_coolwayOpen?.Dispose();DisposeGamlaArt();DisposeObservatoryArt();DisposeMeridianArt();DisposeMineArt();DisposeRegimentArt();DisposeFilm();_doorGround?.Dispose();_doorFace?.Dispose();
