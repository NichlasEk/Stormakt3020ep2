...
public partial class Main
{
    ...
    private void LoadWaterArt()
    {
        ...
        LoadRegimentArt();LoadMineArt();LoadUppsalaArt();
    ...
    private Texture2D RoomBackground=>PaintRoom==Salt.Spring&&_game.RescueState.Briefed?_rescueSpring!:PaintRoom==West.Refuge&&_game.SaltState.Reunited?_saltRefugeOpen!:PaintRoom==Gamla.Registry&&_game.GamlaState.Debriefed?_registryWestOpen!:Gamla.Known(PaintRoom)?_gamlaArt[PaintRoom]:PaintRoom==Observatory.Workshop&&_game.ObservatoryState.CabinetOpened?_workshopEmpty!:Observatory.Known(PaintRoom)?_observatoryArt[PaintRoom]:PaintRoom==Meridian.Hall&&_game.ObservatoryState.EntryOpen?_meridianOpen!:PaintRoom==Cabin.Room?_cabinArt!:_game.InDoorTrial?_doorGround!:PaintRoom==Meridian.Clock?_clockArt!:PaintRoom==Meridian.Hall?_meridianArt!:PaintRoom==Uppsala.Court?(_game.MeridianState.CourtOpen?_uppsalaOpen!:_uppsalaArt!):PaintRoom==Regiment.Quay&&_game.FoundryState.PlateTaken?_quayFrigate!:PaintRoom==Mine.Coolway&&_game.FoundryState.GateOpen?_coolwayOpen!:Mine.Known(PaintRoom)?_mineArt[PaintRoom]:PaintRoom==Regiment.Farled&&_game.MineState.EntranceOpen?_mineFarledOpen!:Regiment.Known(PaintRoom)?_regimentArt[PaintRoom]:PaintRoom switch
