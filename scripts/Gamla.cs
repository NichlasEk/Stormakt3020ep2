...
public static class Gamla
{
    ...
    public static readonly string[] Ids=InitialIds.Concat(West.Ids).ToArray();
    public static bool Known(string id)=>Array.IndexOf(Ids,id)>=0;
    public static string Name(string id)=>West.Known(id)?West.Name(id):id switch{Landing=>"Kungshögarnas uppställningsplats",Passage=>"Gången under högen",_=>"De överfördas väntrum"};
    ...
    public static Vector2[] Ground(string id)=>West.Known(id)?West.Ground:id switch
    {
    ...
    public static Vector2[][] Obstacles(string id)=>West.Known(id)?West.Obstacles(id):id switch
    {
...
public sealed partial class Combat
{
    ...
    {
        if(StepWest(near))return true;if(!InGamla)return false;var s=GamlaState;bool safe=EncounterEnemies.All(e=>e.Dead);
