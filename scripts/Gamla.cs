...
public static class Gamla
{
    ...
    public static readonly string[] Ids={Landing,Passage,Registry};
    public static bool Known(string id)=>Array.IndexOf(Ids,id)>=0;
    public static string Name(string id)=>id switch{Landing=>"Kungshögarnas uppställningsplats",Passage=>"Gången under högen",_=>"De överfördas väntrum"};
...
public sealed partial class Combat
{
    ...
    [JsonIgnore] public string GamlaGoal=>GamlaState.WitnessMet?"Ta Nils vittnesmål till Ebba":Rooms!.Current==Gamla.Landing?!GamlaState.CampRead?"Undersök den övergivna uppställningen":"Följ trappan in under högen":Rooms.Current==Gamla.Passage?!GamlaState.LedgerRead?"Säkra gången · läs transportliggaren":"Fortsätt genom den inre porten":"Säkra väntrummet · tala med Nils";
    [JsonIgnore] public Vector2 GamlaObjective=>GamlaState.WitnessMet?(Rooms!.Current==Gamla.Landing?Gamla.Board:new(100,490)):Rooms!.Current==Gamla.Landing?GamlaState.CampRead?new(1175,495):Gamla.Camp:Rooms.Current==Gamla.Passage?GamlaState.LedgerRead?new(1300,465):Gamla.Ledger:Gamla.Talk;
    ...
    {
        if(!InGamla)return false;var s=GamlaState;bool safe=EncounterEnemies.All(e=>e.Dead);
