...
public partial class Main
{
    ...
    private void LoadJourney()
    {
        ...
        foreach(var (id,line) in JourneyDialogue.Load().Concat(JourneyDialogue.Rooms()).Concat(JourneyDialogue.Archive()).Concat(JourneyDialogue.Roots()).Concat(JourneyDialogue.Regiment()).Concat(JourneyDialogue.Mine()).Concat(JourneyDialogue.Foundry()).Concat(JourneyDialogue.Uppsala()).Concat(JourneyDialogue.Continuity()).Concat(JourneyDialogue.West()).Concat(JourneyDialogue.Gamla()).Concat(JourneyDialogue.Observatory()).Concat(JourneyDialogue.Cabin()).Concat(JourneyDialogue.Meridian()))Radio[id]=(line[0]=="ebba"?"RIKSAMIRAL EBBA GRIP":line[0]=="elin"?"ELIN VINGE":line[0]=="officer"?"MÖNSTRINGSFÖRRÄTTAREN":line[0]=="nils"?"NILS BERG":line[0]=="marta"?"MÄRTA VINGE":line[0]=="meridian"?"MERIDIANVÄKTAREN":line[0]=="bailiff"?"KRONFOGDEN":line[0]=="arvid"?"KAPTEN ARVID SILFVERGREN":"ANTIKVARIE HEDVIG RÅLAMB",line[1]);
    }
