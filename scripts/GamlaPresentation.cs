...
public partial class Main
{
    ...
    {
        if(_nilsArt!=null)return;foreach(var id in Gamla.Ids)_gamlaArt[id]=GD.Load<Texture2D>("res://assets/art/room-"+id+"-v1.png");
    ...
    {
        LoadWaterArt();_gamlaSlot=true;_observatorySlot=_meridianSlot=_uppsalaSlot=_foundrySlot=_mineSlot=_regimentSlot=_doorSlot=_roomsSlot=_portSlot=_atlandSlot=false;
    ...
    {
        if(!_game.InGamla)return false;string text="";var s=_game.GamlaState;
    ...
    private void DisposeGamlaArt(){foreach(var image in _gamlaArt.Values)image.Dispose();_nilsArt?.Dispose();}
}
