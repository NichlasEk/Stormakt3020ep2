...
public partial class Main
{
    ...
    {
        LoadRescueArt();LoadSaltArt();LoadWestArt();if(_nilsArt!=null)return;foreach(var id in Gamla.Ids)_gamlaArt[id]=GD.Load<Texture2D>("res://assets/art/room-"+id+"-v1.png");
    ...
    {
        LoadWaterArt();_rescueSlot=false;_saltSlot=false;_westSlot=false;_gamlaSlot=true;_observatorySlot=_meridianSlot=_uppsalaSlot=_foundrySlot=_mineSlot=_regimentSlot=_doorSlot=_roomsSlot=_portSlot=_atlandSlot=false;
    ...
    {
        if(DrawRescuePrompt())return true;if(DrawSaltPrompt())return true;if(DrawWestPrompt())return true;if(!_game.InGamla||_game.InWest)return false;string text="";var s=_game.GamlaState;
    ...
    private void DisposeGamlaArt(){_ebbaWalk?.Dispose();_ebbaFront?.Dispose();_ebbaBack?.Dispose();_censorArt?.Dispose();_rescueSpring?.Dispose();_karlRescue?.Dispose();_saltArt?.Dispose();_saltWalkArt?.Dispose();_saltRefugeOpen?.Dispose();foreach(var image in _gamlaArt.Values)image.Dispose();_nilsArt?.Dispose();_elinArt?.Dispose();_officerArt?.Dispose();_registryWestOpen?.Dispose();}
}
