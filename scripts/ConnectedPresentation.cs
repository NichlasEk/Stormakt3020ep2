...
public partial class Main
{
    ...
    private void DrawConnectedActors()
    {
        ...
        foreach(var id in ConnectedWorld.RoomIds)
        {
            ...
            AddOathLayers(local);AddArchiveLayers(local);AddRootwayLayers(local);AddRegimentLayers(local);AddMineLayers(local);AddFoundryLayers(local);AddUppsalaLayers(local);AddMeridianLayers(local);AddCabinLayers(local);AddObservatoryLayers(local);AddGamlaLayers(local);AddWestLayers(local);AddSaltLayers(local);AddRescueLayers(local);AddOlderArchLayers(local);AddPaintedArchLayers(local);
    ...
    private void DrawConnectedJournal()
    {
        ...
        if(r.Rooms[Cabin.Room].Visited)DrawLine(At(r.Cabin.ReturnRoom),At(Cabin.Room),Gold,2,true);
        foreach(var id in ConnectedWorld.RoomIds)
        {
    ...
    private void DrawConnectedFog()
    {
        if(_worldFogGame!=_game||_worldFogRevision!=_game.SightRevision)
        {
            ...
            foreach(var id in ConnectedWorld.RoomIds)
            {
                ...
                if(id==West.Refuge&&_game.CanSeeRoomPoint(West.Talk+shift))feet=feet.Append(West.Elin).ToArray();
