...
public partial class Main
{
    private async void RunMusicChecks()
    {
        try
        {
            var before=JsonSerializer.Serialize(_game,new JsonSerializerOptions{IncludeFields=true});ChangeScreen(Screen.Music);SetPhysicsProcess(false);
            ...
            foreach(var t in MusicTracks)
            {
                ...
                if(_sound.CurrentScore!=t.id)throw new Exception("Crossfade target missing");
            ...
            Back();if(_screen!=Screen.Settings)throw new Exception("Music back navigation failed");
            if(before!=JsonSerializer.Serialize(_game,new JsonSerializerOptions{IncludeFields=true}))throw new Exception("Library changed campaign");GD.Print($"MUSIC CHECK PASS: {MusicTracks.Length} streams, crossfades, mute, cinematic silence, library and unchanged campaign");GetTree().Quit();
